...
#ifndef RCLCPP__NODE_PROXY_IMPL_HPP_
#define RCLCPP__NODE_PROXY_IMPL_HPP_
...
#ifndef RCLCPP__NODE_PROXY_HPP_
...
NodeProxy::create_publisher(
  const std::string & topic_name, size_t qos_history_depth,
  std::shared_ptr<Alloc> allocator)
{
  std::string new_topic_name = add_ns_prefix_if_relative(topic_name);
  ...
  std::shared_ptr<Alloc> allocator)
{
  ...
  return original_->create_publisher<MessageT, CallbackT, Alloc, SubscriptionT>(
    new_topic_name,
...
NodeProxy::create_client(
  const std::string & service_name,
  const rmw_qos_profile_t & qos_profile,
  rclcpp::callback_group::CallbackGroup::SharedPtr group)
{
  std::string new_service_name = add_ns_prefix_if_relative(service_name);
...
void
NodeProxy::register_param_change_callback(CallbackT && callback)
{
...
void
NodeProxy::set_parameter_if_not_set(
  const std::string & name,
  const ParameterT & value)
{
  std::string new_parameter_name = add_ns_prefix_if_relative(name);
  original_->set_parameter_if_not_set<ParameterT>(new_parameter_name, value);
}

template<typename ParameterT>
bool
NodeProxy::get_parameter(const std::string & name, ParameterT & value) const
{
  std::string new_parameter_name = add_ns_prefix_if_relative(name);
...
template<typename ParameterT>
void
NodeProxy::get_parameter_or_set(
  const std::string & name,
  ParameterT & value,
...
#endif  // RCLCPP__NODE_PROXY_IMPL_HPP_
